...
eventBus.on('comment.created', safeHandler('comment.created', async (payload) => {
...
eventBus.on('comment.voted', safeHandler('comment.voted', async (payload) => {
...
eventBus.on('comment.approved', safeHandler('comment.approved', async (payload) => {
  await createModerationApprovedNotification(
    payload.commentId,
    payload.authorId
  );
}));
...
eventBus.on('comment.rejected', safeHandler('comment.rejected', async (payload) => {
