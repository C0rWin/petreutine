...
import { closePool, initializeDatabase, query } from './index.js';
...
async function addModerationAlertNotificationType(): Promise<void> {
  try {
    ...
    if (!result.rows[0].exists) {
      ...
      `);
      // Added moderation_alert to notification_type enum
    }
    // Else: moderation_alert already exists in notification_type enum
  } catch (error) {
    ...
    if (errorMessage.includes('does not exist') || errorMessage.includes('already exists')) {
      // notification_type enum update skipped (expected for fresh databases)
    } else {
