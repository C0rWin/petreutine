...
router.get(
  ...
  asyncHandler(async (_req: AuthenticatedRequest, res: Response) => {
    ...
    const stats = await adminStatsCache.getOrFetch(cacheKey, async () => {
      const result = await query<OverviewStats>(`
        SELECT
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    ...
    const cacheKey = `admin:stats:users:${params.days}`;

    const response = await adminStatsCache.getOrFetch(cacheKey, async () => {
      // Basic counts
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const params = statsDateRangeSchema.parse(req.query);
    const cacheKey = `admin:stats:posts:${params.days}`;
...
router.get(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
    const params = statsDateRangeSchema.parse(req.query);
    const cacheKey = `admin:stats:comments:${params.days}`;
