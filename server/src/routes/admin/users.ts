...
import {
  adminUsersQuerySchema, adminPostsQuerySchema, adminCommentsQuerySchema,
  banHistoryQuerySchema, banUserSchema, unbanUserSchema, toggleAdminSchema,
  AdminUserWithStats, AdminPostWithStats, BanHistoryEntry, PaginatedResponse,
  BanType, BanAction, AdminTargetType,
} from '../../types/admin.js';
...
router.get('/', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  if (params.ban_status !== 'all') {
    switch (params.ban_status) {
      case 'banned': whereClause += ` AND u.ban_type IS NOT NULL`; break;
      case 'not_banned': whereClause += ` AND u.ban_type IS NULL`; break;
      case 'comment_banned': whereClause += ` AND u.ban_type = 'comment'`; break;
      case 'full_banned': whereClause += ` AND u.ban_type = 'full'`; break;
    }
  }

  const countResult = await query<{ count: string }>(
    `SELECT COUNT(*) as count FROM users u ${whereClause}`, queryParams
  );
  ...
  res.json({
    data: usersResult.rows, total, limit: params.limit, offset: params.offset,
...
router.get('/:id/posts', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  const params = adminPostsQuerySchema.parse({ ...req.query, user_id: id });

  const countResult = await query<{ count: string }>(
    `SELECT COUNT(*) as count FROM posts WHERE user_id = $1`, [id]
  );
  const total = parseInt(countResult.rows[0].count, 10);
  ...
  res.json({
    data: postsResult.rows, total, limit: params.limit, offset: params.offset,
    has_more: params.offset + params.limit < total,
...
router.get('/:id/comments', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const countResult = await query<{ count: string }>(
    `SELECT COUNT(*) as count FROM comments c ${whereClause}`, queryParams
  );
  const total = parseInt(countResult.rows[0].count, 10);
  ...
  res.json({
    data: commentsResult.rows, total, limit: params.limit, offset: params.offset,
    has_more: params.offset + params.limit < total,
  } as PaginatedResponse<CommentWithUser & { post_title: string }>);
}));

/** GET /:id/ban-history - Get user's ban history */
router.get('/:id/ban-history', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  const params = banHistoryQuerySchema.parse(req.query);

  const countResult = await query<{ count: string }>(
    `SELECT COUNT(*) as count FROM ban_history WHERE user_id = $1`, [id]
  );
  const total = parseInt(countResult.rows[0].count, 10);
  ...
  res.json({
    data: historyResult.rows, total, limit: params.limit, offset: params.offset,
    has_more: params.offset + params.limit < total,
  } as PaginatedResponse<BanHistoryEntry>);
}));

/** POST /:id/ban - Ban a user */
router.post('/:id/ban', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  ...
  const banExpiresAt = input.duration_hours
    ? new Date(Date.now() + input.duration_hours * 60 * 60 * 1000) : null;
  ...
  await logAdminAction(adminId, 'ban_user', AdminTargetType.USER, id, {
    ban_type: input.ban_type, reason: input.reason, duration_hours: input.duration_hours,
  });
...
router.post('/:id/unban', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  const input = unbanUserSchema.parse(req.body);
  const adminId = req.userId!;

  const userCheck = await query<{ ban_type: BanType | null }>(
    'SELECT ban_type FROM users WHERE id = $1', [id]
  );
  if (userCheck.rows.length === 0) throw new AppError('Пользователь не найден', 404);
  ...
  await query(
    `UPDATE users SET ban_type = NULL, ban_reason = NULL, banned_at = NULL,
     banned_by = NULL, ban_expires_at = NULL WHERE id = $1`, [id]
  );
  await query(
...
router.post('/:id/toggle-admin', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  ...
  const adminCheck = await query<{ role: string }>(
    "SELECT role FROM user_roles WHERE user_id = $1 AND role = 'admin'", [id]
  );
  ...
  await logAdminAction(adminId, input.is_admin ? 'grant_admin' : 'revoke_admin',
    AdminTargetType.USER, id, { user_name: userCheck.rows[0].name });
