...
router.get('/', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const countResult = await query<{ count: string }>(
    `SELECT COUNT(*) as count FROM posts p ${whereClause}`, queryParams
  );
...
router.post('/:id/toggle-comments', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  const { id } = req.params;
  ...
  await logAdminAction(adminId, input.enabled ? 'enable_comments' : 'disable_comments',
    AdminTargetType.POST, id, { reason: input.reason });
...
router.delete('/:id', asyncHandler(async (req: AuthenticatedRequest, res: Response) => {
  ...
  const postCheck = await query<{ id: string; title: string; user_id: string }>(
    'SELECT id, title, user_id FROM posts WHERE id = $1', [id]
  );
  if (postCheck.rows.length === 0) throw new AppError('Публикация не найдена', 404);
  ...
  await logAdminAction(adminId, 'delete_post', AdminTargetType.POST, id, {
    title: post.title, user_id: post.user_id, reason: input.reason,
  });
