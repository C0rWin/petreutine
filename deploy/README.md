...
2. **Basic-auth credential** — create `status.env` (gitignored). Compose interpolates
   env_file contents, so the bcrypt `$` must be DOUBLED to `$$`:
   ```bash
   RAW=$(docker run --rm caddy:2-alpine caddy hash-password --plaintext 'YOUR_PASSWORD')
   { echo "STATUS_USER=admin"; echo "STATUS_HASH=${RAW//\$/\$\$}"; } > status.env
   chmod 600 status.env
   # verify Caddy gets a clean single-$ hash after deploy:
   #   docker compose exec caddy printenv STATUS_HASH
   ```
