...
const AppContent: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col">
                      ...
                      <NotificationBadge
                        onNotificationClick={(notification) => {
                          ...
                          if (notification.related_post_id) {
                            const relatedPost = posts.find(p => p.id === notification.related_post_id);
                            if (relatedPost) {
      ...
      <main className="flex-grow max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full">
        ...
        <div className="mb-10 text-center sm:text-left">
            ...
            <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-4 leading-tight">
              Найдите своего <span className="text-transparent bg-clip-text bg-gradient-to-r from-coral-500 to-coral-600">пропавшего друга</span>
            </h1>
            <p className="text-gray-600 text-lg max-w-2xl leading-relaxed">
                ДомойСкорей мгновенно сопоставляет объявления о пропавших и найденных животных в вашем районе. Каждый день питомцы возвращаются домой!
            </p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4 mb-8">
            ...
            <form onSubmit={handleSearch} className="relative w-full sm:w-96">
                ...
                {searchQuery && (
                    <button type="button" onClick={clearSearch} className="absolute right-3 top-3.5 text-gray-400 hover:text-coral-500 transition-colors">
                         <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                    </button>
                )}
            </form>
        ...
        {isLoading || isSearching ? (
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {[...Array(8)].map((_, i) => (
                  <div key={i} className="bg-white rounded-2xl overflow-hidden border border-gray-100 animate-fade-in">
                ...
                {displayPosts.length > 0 ? (
                    ...
                    <div className="text-center py-16 px-8 bg-white rounded-3xl border-2 border-dashed border-gray-200 max-w-2xl mx-auto">
                        ...
                        <p className="text-gray-500 mb-6 max-w-md mx-auto">
                          Попробуйте изменить параметры поиска или создайте новое объявление, чтобы помочь найти вашего питомца.
                        </p>
      ...
      {selectedPost && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setSelectedPost(null)}>
              <div className="bg-white rounded-3xl max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
                  <div className="relative h-64 sm:h-80 bg-gradient-to-br from-gray-100 to-gray-200">
                      ...
                      <button onClick={() => setSelectedPost(null)} className="absolute top-4 right-4 bg-white/20 backdrop-blur-md text-white p-2.5 rounded-xl hover:bg-white/30 transition-colors">
                          <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg>
                      </button>
                  </div>
                  <div className="p-8">
                      <div className="flex justify-between items-start mb-6">
                          <div>
                              <div className="flex items-center gap-2 mb-2 flex-wrap">
                                  <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase text-white shadow-sm ${selectedPost.type === PostType.LOST ? 'bg-coral-500' : 'bg-teal-500'}`}>
                          ...
                          <div className="border-t border-gray-100 pt-6 mb-6">
                              ...
                              {isLoadingMatches ? (
                                  ...
                                  <div className="text-center py-6 bg-gradient-to-br from-gray-50 to-teal-50/30 rounded-xl border border-gray-100">
                                      ...
                                      <p className="text-sm text-gray-500 font-medium">Пока нет подходящих совпадений</p>
                                      <p className="text-xs text-gray-400 mt-1">Совпадения появятся автоматически</p>
                                  </div>
                              )}
                          </div>
                      )}
              ...
              {showDeleteConfirm && (
                  <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60] flex items-center justify-center p-4 animate-fade-in" onClick={() => setShowDeleteConfirm(false)}>
                      <div className="bg-white rounded-2xl p-6 max-w-sm w-full shadow-2xl animate-slide-up" onClick={e => e.stopPropagation()}>
                          ...
                          <h3 className="text-xl font-bold text-gray-900 mb-2 text-center">Удалить объявление?</h3>
                          <p className="text-gray-500 mb-6 text-center text-sm">Это действие нельзя отменить. Объявление будет удалено навсегда.</p>
